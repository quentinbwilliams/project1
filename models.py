...
class Team(db.Model):
    ...
    # league = db.Column(db.Text, db.ForeignKey('league.name'), nullable=False)
    
    @staticmethod
    def get_player_stats(self):
        """  """ 
        url = "https://api-football-v1.p.rapidapi.com/v3/players"
        querystring = {"team":f"{self.api_id}","season":f"{season}"}
        headers = {
            'x-rapidapi-host': "api-football-v1.p.rapidapi.com",
            'x-rapidapi-key': f"{API_FOOTBALL_KEY}"
            }
    ...
    def get_team_stats(self):
        ...
        querystring = {"league":f"{self.league_id}","season":f"{season}", "team":f"{self.api_id}"}
